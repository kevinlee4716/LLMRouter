 ...
 *   `llmrouter.locale` and falls back to `navigator.language` on first visit
...
function detectLocale(): Locale {
  ...
  const stored = window.localStorage.getItem('llmrouter.locale')
...
export function I18nProvider({ children, initialLocale }: I18nProviderProps) {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('llmrouter.locale', locale)
  ...
  const value = useMemo<I18nContextValue>(() => {
    ...
    return {
      ...
      t: (key, vars) => {
        ...
        // Fallback to en so a partial zh-CN still renders something.
