...
import { z } from 'zod';
import type { ChatMessage, ChatToolCall, ModelListRow } from '@llmrouter/shared/types.js';
...
// on every request, but LLMRouter's whole point is to pick the model itself.
...
proxyRouter.post('/chat/completions', async (req: Request, res: Response) => {
  ...
  // per-request `X-LLMRouter-Cache` header can force or bypass. Off unless enabled
  // via the RESPONSE_CACHE env var or the response_cache_enabled setting.
  const cacheDirective = parseCacheDirective(req.headers['x-llmrouter-cache'], req.headers['cache-control']);
  ...
  if (cacheKey) {
    ...
    if (hit) {
      ...
      res.setHeader('X-LLMRouter-Cache', 'HIT');
      res.json(hit.body);
      return;
    }
  }
  ...
  await runFallbackLoop({
    ...
    if (handoffMode !== 'off' && sessionKey) {
      ...
      if (stream) {
        ...
        res.setHeader('X-LLMRouter-Cache', cacheKey ? 'MISS' : 'OFF');
