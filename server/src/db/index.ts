...
const DB_PATH = path.resolve(__dirname, '../../data/llmrouter.db');
...
export function getDefaultDbPath(): string {
  return process.env.LLMROUTER_DB_PATH?.trim() || DB_PATH;
}
...
export function connectDb(
  ...
  if (!isMemory) {
    ...
    db.pragma('synchronous = NORMAL');       // safe with WAL, reduces I/O
    db.pragma('cache_size = -2000');          // 2 MB cache — sufficient for local proxy
  }
...
export function regenerateUnifiedKey(): string {
  const db = getDb();
  const key = `llmrouter-${crypto.randomBytes(24).toString('hex')}`;
