...
//     sets LLMROUTER_BLOCK_PRIVATE_PROVIDER_URLS=true (recommended for
...
function blockPrivateEnabled(): boolean {
  return /^(1|true|yes)$/i.test(process.env.LLMROUTER_BLOCK_PRIVATE_PROVIDER_URLS ?? '');
}
...
export async function assessProviderUrl(rawUrl: string, opts: AssessOptions = {}): Promise<UrlAssessment> {
  ...
  for (const address of addresses) {
    ...
    if (blockPrivate && (cls === 'loopback' || cls === 'private')) {
      return {
        allowed: false,
        reason: `resolves to a private address (${address}) and LLMROUTER_BLOCK_PRIVATE_PROVIDER_URLS is set`,
      };
    }
  }
  return { allowed: true };
