...
export function logRequest(
  ...
  try {
    ...
    const tx = db.transaction(() => {
      ...
      // Record request ID for cost tracking (outside the tx to avoid SQLite contention)
      const requestId = insert.lastInsertRowid;
      return { requestId, isSuccess: status === 'success' };
    });
    const { requestId, isSuccess } = tx() as { requestId: number; isSuccess: boolean };

    pruneRequestAnalytics({ db });

    // Hook: cost tracking & channel health (fire-and-forget, don't block)
    if (isSuccess && keyId > 0) {
      try {
        import('../services/cost-tracking.js').then(({ recordCost }) => {
          recordCost(requestId as number, keyId, platform, modelId, inputTokens, outputTokens);
        }).catch(() => {});
      } catch { /* ignore */ }
    }
    try {
      import('../services/channel-health.js').then(({ recordChannelResult }) => {
        recordChannelResult(keyId, platform, modelId, isSuccess, error || undefined);
      }).catch(() => {});
    } catch { /* ignore */ }
  } catch (e) {
