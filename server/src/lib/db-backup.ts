...
const MAGIC = Buffer.from('LMRBK1\0');
...
function backupTarget(): string | null {
  const raw = process.env.LLMROUTER_DB_BACKUP_TARGET
    ?? process.env.LLMROUTER_DB_BACKUP_URL
    ?? process.env.LLMROUTER_DB_BACKUP_PATH
    ?? '';
...
function backupIntervalMs(): number {
  const raw = process.env.LLMROUTER_DB_BACKUP_INTERVAL_MS;
...
function parseBackupKey(): Buffer {
  const raw = (process.env.LLMROUTER_DB_BACKUP_KEY || process.env.ENCRYPTION_KEY || '').trim();
  if (!raw || raw === PLACEHOLDER_KEY || !/^[0-9a-fA-F]{64}$/.test(raw)) {
    throw new Error('LLMROUTER_DB_BACKUP_KEY or ENCRYPTION_KEY must be a 64-character hex key when DB backup is enabled');
  }
...
async function readTarget(target: string): Promise<Buffer | null> {
  if (isHttpTarget(target)) {
    ...
    const token = process.env.LLMROUTER_DB_BACKUP_TOKEN?.trim();
