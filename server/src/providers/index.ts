...
register(new OpenAICompatProvider({
  ...
  extraHeaders: {
    'HTTP-Referer': 'http://localhost:2210',
    'X-Title': 'LLMRouter',
  },
}));
...
// shipped as LLMRouter model migrations.
register(new OpenAICompatProvider({
  platform: 'agnes',
  name: 'Agnes AI',
...
// 阿里云百炼 (Model Studio) — OpenAI-compatible via
// dashscope.aliyuncs.com/compatible-mode/v1. Supports Qwen and DeepSeek series.
// Free tier: 100万 tokens/mo for Qwen-Turbo, 200万 tokens/mo for Qwen-Plus
// (new users, valid for 6 months). Key from bailian.console.aliyun.com.
register(new OpenAICompatProvider({
  platform: 'aliyun',
  name: '阿里云百炼',
...
// 百度千帆 (Qianfan) — OpenAI-compatible via qianfan.baidubce.com v2
// endpoint. Baidu post-2025 fully uses the v2 OpenAI-compatible interface.
// Free tier includes ERNIE-Speed (ERNIE-Lite) and ERNIE-Tiny series with
// generous RPM/TPM quotas. Key from console.bce.baidu.com/qianfan.
register(new OpenAICompatProvider({
  platform: 'qianfan',
  name: '百度千帆',
...
register(new OpenAICompatProvider({
  ...
  extraHeaders: {
    'User-Agent': 'Mozilla/5.0 LLMRouter/1.0',
  },
}));
 ...
 * the caller-supplied base URL (stored per api_keys row). Custom vendor
 * platforms from provider_vendors are resolved dynamically by looking up their
 * api_base_url. Returns undefined for an unknown platform or a custom vendor
...
export function resolveProvider(platform: Platform, baseUrl?: string | null): BaseProvider | undefined {
  ...
  // Check the built-in provider registry first
  const builtin = providers.get(platform);
  if (builtin) return builtin;

  // Fallback: look up a user-defined custom vendor from provider_vendors
  try {
    const db = getDb();
    const vendor = db.prepare(
      'SELECT name, api_base_url FROM provider_vendors WHERE platform = ? AND is_system = 0'
    ).get(platform) as { name: string; api_base_url: string } | undefined;
    if (vendor?.api_base_url) {
      return new OpenAICompatProvider({
        platform: platform as Platform,
        name: vendor.name,
        baseUrl: vendor.api_base_url,
        timeoutMs: CUSTOM_PROVIDER_TIMEOUT_MS,
      });
    }
  } catch {
...
export function hasProvider(platform: Platform): boolean {
  ...
  // Check user-defined custom vendors from provider_vendors
  try {
    const db = getDb();
    const row = db.prepare(
      'SELECT 1 FROM provider_vendors WHERE platform = ? AND is_system = 0 AND api_base_url != \'\''
    ).get(platform);
    return !!row;
  } catch {
    return false;
