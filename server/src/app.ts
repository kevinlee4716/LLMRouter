...
import { customProvidersRouter } from './routes/custom-providers.js';
import { cacheRouter } from './routes/cache.js';
import { channelHealthRouter } from './routes/channel-health.js';
import { guardrailsRouter } from './routes/guardrails.js';
import { webhooksRouter } from './routes/webhooks.js';
import { modelRemappingRouter } from './routes/model-remapping.js';
import { announcementsRouter } from './routes/announcements.js';
import { costTrackingRouter } from './routes/cost-tracking.js';
import { tokenQuotasRouter } from './routes/token-quotas.js';
...
export function createApp(config?: Config) {
  ...
  // 2mb — sufficient for local use with chat messages + tool schemas.
  // Larger limits (10mb) were only needed for code agents shipping repo
  // context, which isn't a supported deployment on localhost.
  ...
  app.use('/api/channel-health', requireAuth, channelHealthRouter);
  app.use('/api/guardrails', requireAuth, guardrailsRouter);
  app.use('/api/webhooks', requireAuth, webhooksRouter);
  app.use('/api/model-remapping', requireAuth, modelRemappingRouter);
  app.use('/api/announcements', requireAuth, announcementsRouter);
  app.use('/api/cost-tracking', requireAuth, costTrackingRouter);
  app.use('/api/token-quotas', requireAuth, tokenQuotasRouter);
