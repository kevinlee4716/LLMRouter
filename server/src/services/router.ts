...
function selectKeyForModel(entry: ChainRow, estimatedTokens: number, skipKeys?: Set<string>, diag?: string[]): RouteResult | null {
  ...
  // Custom vendor platforms (from provider_vendors) may have a registered
  // provider via hasProvider (lookup) but getProvider returns undefined for
  // dynamically-resolved vendors. We'll resolve them per-key below with
  // the key's stored base_url — the same code path used for 'custom'.
  const isCustomVendor = !provider; // platform is only known via provider_vendors
  ...
  for (let attempt = 0; attempt < keys.length; attempt++) {
    ...
    const resolvedProvider = entry.platform === 'custom' || isCustomVendor
      ? resolveProvider(entry.platform as Platform, key.base_url)
      : provider;
