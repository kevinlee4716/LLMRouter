...
function readConfigFromEnv(): { source: string; value: unknown } | null {
  const inline = process.env.LLMROUTER_CONFIG_JSON?.trim();
  if (inline) return { source: 'LLMROUTER_CONFIG_JSON', value: JSON.parse(inline) };

  const configPath = process.env.LLMROUTER_CONFIG_PATH?.trim();
